...
DEFAULT_TEMP = meta["temperature"]
DEFAULT_MAX_TOKENS = meta["max_tokens"]

# --- INPUT VALIDATION HELPERS --- #

def pick(prompt: str, choices: dict, allow_blank=None):
    """Prompt until a valid menu choice. Returns the chosen key (int),
    the string 'X' to bail, or `allow_blank` if Enter is hit empty.
    Never raises on bad input — re-asks instead."""
    while True:
        raw = input(prompt).strip()
        if raw.upper() == "X":
            return "X"
        if raw == "" and allow_blank is not None:
            return allow_blank
        try:
            n = int(raw)
        except ValueError:
            console.print(
                f"'{raw}' isn't a number — pick from the list or X.",
                style="bold red"
            )
            continue
        if n not in choices:
            console.print(
                f"{n} is out of range (valid: {min(choices)}-{max(choices)}).",
                style="bold red"
            )
            continue
        return n

def ask_number(label: str, current, cast):
    """Enter keeps `current`; a valid number replaces it; X bails;
    anything else re-asks. Never raises."""
    while True:
        ...
        try:
            ...
            console.print(
                f"'{raw}' isn't a valid number — Enter keeps {current}.",
                style="bold red"
            )
            continue
...
def beautify(answer:str, model_name:str, timestamp:str):
...
def save_transcript(messages, tone, name, mode, timestamp):
    """Shared save logic for both modes. Returns the path written."""
    entries = []
    for dic in messages:
        if dic["role"] == "system":
            pass
...
def salvage(state):
    """Last-ditch dump of the conversation on unexpected crash."""
    messages = state.get("messages") or []
    if not messages:
        return
    stamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    crash_log = SAVE_DIR / f"CRASH_{stamp}_{state['name']}_{state['tone']}.md"
    body = "".join(
        f"\n**{m['role'].upper()}:**\n{m['content']}\n\n---\n"
        for m in messages if m["role"] != "system"
    )
    crash_log.write_text(f"# Salvaged session {stamp}\n{body}", encoding="utf-8")
    console.print(f"Session salvaged to {crash_log}", style="bold yellow")
...
def main(state):
    while True:
        ...
        if m == "X":
            break
        model = model_choices[m]

        # per-model run params start from TOML defaults
        temp = DEFAULT_TEMP
        ...
        while True:
            ...
            if mode in ["0", ""]:
                ...
                t = pick("Tone Selection:\n~ ", tone_choices, allow_blank="default")
                if t == "X":
                    break
                tone = t if t == "default" else tone_choices[t]
                state["tone"] = tone
                ...
                while True:
                    ...
                    budget = ask_number("Max Tokens", max_tokens, int)
                    if budget == "X":
                        break
                    max_tokens = budget

                    # custom temp setting
                    t_in = ask_number("Temperature", temp, float)
                    if t_in == "X":
                        break
                    temp = t_in
                    ...
                    beautify(answer, name, timestamp)
            ...
            if user.upper() not in ("Y",""):
                ...
                log_file = save_transcript(messages, tone, name, mode, timestamp)
...
if __name__ == "__main__":
    state = {"name": "unknown", "tone": "default", "messages": []}
    try:
        main(state)
